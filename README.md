...
**v0.3은 한 명의 소유자가 사용하는 로컬 파일럿입니다.** 설계를 macOS 본인 인증으로 승인한 뒤, Claude Code가 격리된 Docker 작업 공간에서 구현하고 고정 검사와 별도 읽기 전용 리뷰를 수행합니다. 여러 프로젝트의 실행·diff·검사 근거를 앱에서 확인합니다. 실제 팀 계정과 서버 운영은 후속 M3 범위입니다.
...
외부 배포 준비 상태·설치 파일 검증·남은 결정은 [배포 안내](docs/DISTRIBUTION.md)를 확인하세요. `0.3.0-beta.1`은 품질 보강 중인 베타 후보이며 아직 공개 배포 완료 버전이 아닙니다.
