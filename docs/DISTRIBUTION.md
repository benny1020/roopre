# 외부 배포 준비 · 0.2.1-beta.1

상태: **배포 전 품질 보강 중인 로컬 베타 후보**. 사용자 요청은 외부 배포 가능한 수준으로 품질을 높이는 것이며, 공개 게시·머지·유료 서비스 구매를 승인한 것은 아니다. 기존 M2 구조와 본인 설계 승인 원칙을 유지한다. 공개 대상·Apple Developer 가입 여부 질문의 답변을 기다리는 동안 소수 개발자 비공개 베타를 제안 기준으로 삼는다.

## 이번 보강 범위

| 입력/실패                                | 프로그램의 처리                                                          | 확인할 결과                                            |
| ---------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------ |
| 시작 시 DB 중단                          | 제한 시간 내 실패, pool 정리, 설치 안내/재연결/종료 선택                 | 원본 데이터 보존, 두 번 재연결해도 정상 종료           |
...
| 깨끗한 다른 Mac 설치 | 소스·Node·pnpm 없는 Mac에 ZIP 설치, Git/Docker/DB 준비, 첫 작업 완료, 재시작 복원 확인 필요                                                                                  |
| DB 자격 증명         | 현재 개발용 고정 비밀번호/로컬 공유 볼륨 사용. 같은 Mac의 사용자·프로세스를 신뢰하는 파일럿 전제. 공개 배포 전 사용자별 DB 자격 증명 생성 및 기존 데이터 이전 설계 승인 필요 |
...
| 머지                 | 전담 PR 리뷰와 최신 CI 이후 본인 승인. 기존 승인으로 새 커밋을 머지하지 않음                                                                                                 |
