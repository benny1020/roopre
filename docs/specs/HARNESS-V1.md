...
프로필의 선택적 `execution` 객체에 `requirements`, `design`, `implementation`, `verification`, `review`별 `parallel` 또는 `sequential`을 지정한다. 생략된 단계는 새 실행에서 기본 병렬이다. 최대 3개씩 독립 환경에서 실행하고 단계 사이에서는 전체 결과를 기다린다. `execution`은 프로젝트에 적용되고 export/import와 lock digest에 포함된다. 예전 실행 스냅샷의 누락 필드는 순차로 해석한다. 실행 의미 변경에 따라 기존 설계는 새 게시·본인 승인이 필요하다. 자세한 결과 통합과 실패 기준은 [병렬 실행 설계](../design/PARALLEL-STAGES.md)를 따른다.
