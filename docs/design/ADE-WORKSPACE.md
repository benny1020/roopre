...
Independent review found two blocking interaction/evidence regressions, both corrected: global commands no longer stack a second dialog over a creation form, and AI review results are partitioned by attempt with input/output tree and required/optional identity. Regression scenarios verify focus remains usable with the draft preserved, and an old passing acceptance review stays explicitly historical during repair. Live agent selection also excludes stale `running` records from previous attempts; unconfirmed cancellation is shown as awaiting termination.
