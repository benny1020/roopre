...
export function apply(
  ...
  switch (c.type) {
    ...
    case "review": {
      ...
      requireThat(
        actor.id !== f!.authorId && d.reviewers.includes(actor.id),
        "reviewer_required",
    ...
    case "queue_run": {
      ...
      requireThat(
        !w.runs.some(
          (r) =>
            r.featureId === f!.id && ["queued", "blocked"].includes(r.status),
        ),
        "duplicate_run",
        "이미 실행 대기 중인 기능입니다.",
      );
